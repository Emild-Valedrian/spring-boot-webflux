package com.NetX.springboot.webflux.app.controllers;
...
public class ProductoController {
	...
	public Mono<String> ver (Model model, @PathVariable String id){
		...
		return service.findById(id)
				...
				.flatMap(p -> {
					if (p.getId() == null) {
						return Mono.error(new InterruptedException("Error: NO EXISTE"));
					}
					return Mono.just(p);
				}).then(Mono.just("ver"))
	...
	public Mono<String> eliminar(@PathVariable String id) {
		return service.findById(id).defaultIfEmpty(new Producto()).flatMap(p -> {
			if (p.getId() == null) {
				return Mono.error(new InterruptedException("Error: El producto a eliminar no existe"));
			}
			return Mono.just(p);
